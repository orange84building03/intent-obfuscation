...
knitr::opts_chunk$set(
  ...
  fig.path = "rmd_imgs/", root.dir = here()
)
```
...
success_intended_data <- success_raw_data |>
  mutate(success_intended_count = case_when(loss_target == "Mislabeling" ~ mislabel_intended_count,
...
g +
  labs(x = itr_lab, y = glue("p(Success) {norm_cap(params$norm)}"), color = "Attack", linetype = "Attack") +
...
cap <- table_caption("attacks, split by detection models", "Targeted attacks obtain higher success than untargeted attacks on YOLOv3 only. Within targeted attacks, vanishing attacks obtain higher success than mislabeling attacks on all models")
