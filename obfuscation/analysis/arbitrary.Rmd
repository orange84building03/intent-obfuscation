...
params:
  data: "/Users/zbli/Documents/Documents - ZhaoBin’s MacBook Pro/repo/adversarial/adversarial data" # directory where data is stored
...
knitr::opts_chunk$set(
  ...
  root.dir = here(), fig.path = "rmd_imgs/"
)
```
...
success_fnames <-
  dir_ls(data_dir, glob = glue("*{params$norm}*.csv"))
...
#   count(model_name, loss_target, bbox_length, boundary_distance) |>
#   glimpse()
# 
# stopifnot(unique(reps$n) == 2)
# 
# reps <- success_raw_data |>
#   group_by(model_name, loss_target, bbox_length, boundary_distance) |>
#   summarize(total_samples = sum(attack_count)) |>
#   glimpse()
# 
# oks <- all(reps$total_samples == 200)
# 
# if (!oks) {
#   reps |>
#     filter(total_samples != 200) |>
#     glimpse()
#   stop()
# }
```
...
len_lab <- "Perturb Box Length (relative to image width/height)"
...
g + labs(x = dist_lab, y = glue("p(Success) {norm_cap(params$norm)}")) +
